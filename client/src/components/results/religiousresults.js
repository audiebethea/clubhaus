...
export default class ReligiousResults extends React.Component{
    ...
    componentDidMount(){
        ...
        if(this.props.religiousAnswers[0] === "Not Interested"){
            returnedClubs = [];
            this.forceUpdate();
        }
        else{
    ...
    async getClubs(){
        ...
        const query = '/filter/religious/' + university;
        ...
        try{
            const response = await fetch(query, {
                ...
                body: JSON.stringify(this.props.religiousAnswers)
            });
            if(response.ok){
